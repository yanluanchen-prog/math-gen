...
## 输出模板（本项目默认：证据索引格式｜你选的 -3）

对每个 URL 输出一条“证据卡”，用于直接粘贴/沉淀到飞书「证据索引」：
...
- 摘要要点（3条，<=12字/条）：
  ...
  2)
  3)
- 可引用原文（最多3条，每条<=120字，尽量原句）：
  - 引用1："..."
  - 引用2："..."
  - 引用3："..."
- 元信息（能取就取）：平台｜标题｜作者｜发布时间｜读取方式
- 状态：success/failed（失败写原因+下一步建议）

> 批量链接时，按“证据卡”逐条输出，便于直接汇总成索引。
